...
def processSingleQuery(imagefile, rindex, outfile, mode="N_GRAM"):
    ...
    if mode == "NORMAL":
        ...
        offsetDict = getOffsetDeltaNGram(bscore_query, rindex, N_Gram=2)
...
def processQuery_wrapper(queryfile, rindex, outdir, mode):
    ...
    return processSingleQuery(queryfile, rindex, hyp_outfile, mode)


status = 'train'
pickle_file = 'experiments/indices/N_GRAM_2_ALL.pkl'
query_list = 'cfg_files/query.'+status+'.list'
outdir = 'experiments/Analysis_2_GRAM/'+status
mode = "N_GRAM"
inputs = []
...
for i in inputs:
    processQuery_wrapper(i[0], rindex, i[1], mode = mode)
